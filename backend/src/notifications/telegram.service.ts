...
export class TelegramService {
  ...
  async sendMessage(message: string, overrideConfig?: { token: string; chatId: string }) {
    ...
    if (!token || !chatId) {
      ...
      const settings = await this.prisma.systemSetting.findMany({
        where: { key: { in: ['telegramBotToken', 'telegramChatId', 'notifyTelegram'] } },
      });

      const notifyTelegram = settings.find((s) => s.key === 'notifyTelegram')?.value === 'true';
      if (!notifyTelegram) {
        this.logger.debug('Telegram notifications are disabled globally. Skipping message.');
    ...
    if (!token || !chatId) {
      this.logger.debug('Telegram Bot Token or Chat ID is not configured. Skipping message.');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return { success: false, message: data.description || 'Unknown Telegram API Error' };
      }
