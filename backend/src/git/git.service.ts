...
export class GitService {
  ...
  cloneRepository(
    ...
    branch: string, 
    cloneDir: string, 
    credentials?: { sshPrivateKey?: string | null; githubToken?: string | null }
  ): void {
    ...
    } else if (credentials?.githubToken && repositoryUrl.startsWith('https://')) {
    ...
    try {
      execSync(`git clone --depth 1 --branch ${branch} ${finalUrl} ${cloneDir}`, {
        timeout: 120000,
        stdio: 'pipe',
        env,
      });
