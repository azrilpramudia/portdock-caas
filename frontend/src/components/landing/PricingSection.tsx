...
export default function PricingSection() {
  return (
    <section className="py-24 px-6 bg-slate-50/50" id="pricing">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {LANDING_PRICING.map((plan, index) => (
            <div 
              key={plan.name} 
              className={`relative bg-white rounded-3xl p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-transform duration-300 hover:-translate-y-1 flex flex-col ${
                plan.highlight ? 'border-2 border-blue-500' : 'border border-slate-100'
              }`}
            >
              {plan.highlight && (
